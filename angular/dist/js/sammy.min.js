...
"use strict";app.config(["$stateProvider","$urlRouterProvider",function(t,e){e.otherwise("/"),t.state("index",{url:"/",templateUrl:"templates/dashboard.html"}).state("cars",{url:"/cars",templateUrl:"templates/car_overview.html"}).state("cardetails",{url:"/cardetails",templateUrl:"templates/car_details.html"})}]);
...
function CarsCtrl(r,t,a,c){t.get(a.url+"/cars").success(function(t,a,c,n){r.cars=t}).error(function(r,t,a,n){c.addAlert("danger","fa-exclamation-triangle","Can't fetch car data")})}app.controller("CarsCtrl",["$scope","$http","Config","Alert",CarsCtrl]);
function CarDetailsCtlr(t,l,r,a,e){t.testVal="test"}app.controller("CarDetailsCtlr",["$scope","$http","$interval","Config","Alert",CarDetailsCtlr]);
function DashboardCtrl(r,o,t,a){}app.controller("DashboardCtrl",["$scope","$http","Config","Alert",DashboardCtrl]);
function HeaderCtrl(e,t,r,a,n){e.update=function(){var t=r.url();switch(t){case"/cars":e.currentPage="Mijn voertuigen";break;case"/cardetails":e.currentPage="Mijn voertuig details";break;default:e.currentPage="Login"}e.crumbs=[],e.crumbs=t.split("/")},e.update(),t.$on("$stateChangeStart",function(t,r,a,n,c){e.update()})}app.controller("HeaderCtrl",["$scope","$rootScope","$location","Config","$http",HeaderCtrl]);
