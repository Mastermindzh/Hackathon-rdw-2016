...
"use strict";app.config(["$stateProvider","$urlRouterProvider",function(t,e){e.otherwise("/"),t.state("index",{url:"/",templateUrl:"templates/dashboard.html"}).state("cars",{url:"/cars",templateUrl:"templates/car_overview.html"})}]);
...
function CarsCtrl(t,r,l,C,a){t.testVal="test"}app.controller("CarsCtrl",["$scope","$http","$interval","Config","Alert",CarsCtrl]);
function DashboardCtrl(r,o,t,a){}app.controller("DashboardCtrl",["$scope","$http","Config","Alert",DashboardCtrl]);
function HeaderCtrl(t,e,r,a,n){t.update=function(){var e=r.url();switch(e){case"/cars":t.currentPage="Mijn voertuigen";break;default:t.currentPage="Login"}t.crumbs=[],t.crumbs=e.split("/")},t.update(),e.$on("$stateChangeStart",function(e,r,a,n,o){t.update()})}app.controller("HeaderCtrl",["$scope","$rootScope","$location","Config","$http",HeaderCtrl]);
